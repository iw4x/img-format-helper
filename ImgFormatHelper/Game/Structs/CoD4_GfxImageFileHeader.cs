...
namespace IWImgViewer
{
    public class CoD4_GfxImageFileHeader : IImageFileHeader
    {
        ...
        public byte MipLevels => (byte)((flags & (byte)Iwi6Flags.IMG_FLAG_NOMIPMAPS) == 0 ? 0 : 1 + Math.Floor(Math.Log(Math.Max(dimensions[0], Math.Max(dimensions[1], dimensions[2])), 2)));
        ...
        public MapType MapType => (flags & (int) Iwi6Flags.IMG_FLAG_CUBEMAP) == 0 ? MapType.MAPTYPE_2D : MapType.MAPTYPE_CUBE;
        ...
        public void From(IImageFileHeader header)
        {
            ...
            var textureMipCount = (flags & (uint)Iwi6Flags.IMG_FLAG_NOMIPMAPS) == 0 ? 1 : header.MipLevels;
